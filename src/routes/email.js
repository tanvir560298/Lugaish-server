...
router.get('/status', ...manageEmail, async (req, res) => {
  try {
    if (req.isTester) {
      return res.json({ configured: true, connected: true, senderEmail: 'tester-sandbox@lugaish.local', maxRecipientsPerCampaign: MAX_RECIPIENTS_PER_CAMPAIGN, recentCampaigns: [], sandbox: true });
    }
    ...
    const recentCampaigns = await EmailCampaign.find({})
      .select('subject recipientCount sentCount failedCount status createdAt')
      .sort({ createdAt: -1 })
      .limit(10);
    res.json({
      configured: Boolean(config.GMAIL_CLIENT_ID && config.GMAIL_CLIENT_SECRET && config.GMAIL_TOKEN_ENCRYPTION_KEY),
      connected: Boolean(connection),
      senderEmail: connection?.senderEmail || config.GMAIL_SENDER_EMAIL,
...
router.get('/oauth/url', ...manageEmail, (req, res) => {
  try {
    if (req.isTester) {
      return res.json({ url: `${config.FRONTEND_URL.replace(/\/$/, '')}/dashboard?mail=tester-preview`, sandbox: true });
    }
...
router.get('/oauth/callback', async (req, res) => {
  try {
    ...
    if (!canManageEmail(user)) {
      throw new Error('You no longer have permission to connect Gmail');
    }
    if (normalizeRole(user.role) === ROLES.tester) {
      return res.redirect(`${config.FRONTEND_URL.replace(/\/$/, '')}/dashboard?mail=tester-preview`);
    }
    ...
    await MailConnection.findOneAndUpdate(
      ...
      {
        key: 'primary',
        senderEmail: config.GMAIL_SENDER_EMAIL,
    ...
    res.redirect(`${config.FRONTEND_URL.replace(/\/$/, '')}/dashboard?mailError=${message}`);
  }
});
...
router.post('/campaigns', ...manageEmail, async (req, res) => {
  try {
    ...
    if (confirmation !== 'SEND TO ALL USERS') return res.status(400).json({ error: 'Campaign confirmation text does not match' });
    if (req.isTester) {
      return res.json({
        message: 'Tester campaign preview complete. No recipients were contacted.',
        campaign: { subject, recipientCount: 0, sentCount: 0, failedCount: 0, status: 'sandbox' },
        sandbox: true,
      });
    }
    const lockKey = String(req.userId);
    if (campaignLocks.has(lockKey)) {
      return res.status(409).json({ error: 'Another campaign is already sending. Wait for it to finish.' });
    }
    ...
    try {
      ...
      if (users.length > MAX_RECIPIENTS_PER_CAMPAIGN) {
        return res.status(409).json({ error: `Safety limit is ${MAX_RECIPIENTS_PER_CAMPAIGN} recipients per campaign. Use a dedicated bulk email provider for larger lists.` });
      }
      if (!users.length) return res.status(400).json({ error: 'No recipients found' });

      const campaign = await EmailCampaign.create({
        subject, message, senderEmail: config.GMAIL_SENDER_EMAIL, createdBy: req.userId, recipientCount: users.length,
      });
      const gmail = await getAuthorizedGmail();
      const failures = [];
      let sentCount = 0;
      ...
      for (let index = 0; index < users.length; index += 3) {
        ...
        const results = await Promise.allSettled(batch.map(user => gmail.users.messages.send({
          userId: 'me',
          requestBody: { raw: createRawMessage({ to: user.email, name: user.name, subject, message }) },
        })));
